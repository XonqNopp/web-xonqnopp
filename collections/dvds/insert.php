...
if(isset($_POST["title"])) {
	if(isset($_POST["erase"])) {
		$id = $_POST["id"];
		$page->dbHelper->idManage("DELETE FROM `{$page->dbHelper->dbName}` . `dvds` WHERE `dvds` . `id` = ? LIMIT 1;", $id);
		...
		$query = "";
		if($id > 0) {
			$query = $page->dbHelper->queryPrepare("UPDATE `{$page->dbHelper->dbName}` . `dvds` SET `title` = ?, `director` = ?, `actors` = ?, `languages` = ?, `subtitles` = ?, `duration` = ?, `serie` = ?, `number` = ?, `summary` = ?, `burnt` = ?, `format` = ?, `category` = ? WHERE `dvds` . `id` = ? LIMIT 1;");
			...
			$query = $page->dbHelper->queryPrepare("INSERT INTO `{$page->dbHelper->dbName}` . `dvds` (`id`, `title`, `director`, `actors`, `languages`, `subtitles`, `duration`, `serie`, `number`, `summary`, `burnt`, `format`, `category`) VALUES(NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
...
if(isset($_GET["id"]) || isset($_POST["id"])) {
	...
	// Fetch infos from DB
	$query = $page->dbHelper->idManage("SELECT * FROM `{$page->dbHelper->dbName}` . `dvds` WHERE `dvds` . `id` = ? LIMIT 1;", $id);
	$query->store_result();
	if($query->num_rows == 0) {
		$query->close();
		exit("Error bad id");
...
// Title
$titleAttr = FieldAttributes(true, true);
$titleAttr->size = 60;

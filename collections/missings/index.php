...
function getMissingsFromBorrower($person) {
    ...
    $body .= $page->butler->cell(
        //"<a id=\"borrower$borrowerId\">{$person->name} ({$missings->num_rows})</a>",
        "{$person->name} ({$missings->num_rows})",
        array("class" => "missing_display_name$cssborrower", "colspan" => 2, "id" => "borrower$borrowerId")
        // TODO check if anchor working without <a>
    );

...
if(isset($_POST["dbid"])) {
	...
	$missing = $page->dbHelper->queryPrepare("INSERT INTO `{$page->dbHelper->dbName}` . `missings` (`id`, `borrower`, `dbtable`, `dbid`, `when`) VALUES(NULL, ?, ?, ?, ?)");
	$missing->bind_param("isis", $_POST["borrower"], $dbtable, $dbid, $_POST["date"]);
	$page->dbHelper->executeManage($missing);

	$otherdb = $page->dbHelper->queryPrepare("UPDATE `{$page->dbHelper->dbName}` . `{$dbtable}` SET `borrowed` = 1 WHERE `id` = ? LIMIT 1;");
	$otherdb->bind_param("i", $dbid);
	$page->dbHelper->executeManage($otherdb);
...
$body = $page->bodyHelper->goHome("..", "../$dbtable/index.php");


// Fetch item info
$item = $page->dbHelper->idManage("SELECT * FROM `$dbtable` WHERE `id` = ?", $dbid);
$item->store_result();
...
if($dbtable == "bds") {
	...
	$serie = "";
	if($serie_id > 0) {
		$getserie = $page->dbHelper->queryManage("SELECT * FROM `bdseries` WHERE `id` = $serie_id");
...
$attributes = FieldAttributes(false, true);

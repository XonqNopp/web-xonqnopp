...
function getBody($result) {
    ...
    while($row = $result->fetch_object()) {
        ...
        if($userIsAdmin) {
            $body .= $page->butler->cell($page->bodyBuilder->anchor("insert.php?id=$quoteId", "edit"), array("class" => "editbutton"));
        //
            $coroner = ($fav ? "un" : "") . "favorize";
            $body .= $page->butler->cell(
                $page->bodyBuilder->anchor("index.php?NewFav=" . ($fav ? "-" : "") . "$quoteId#c$quoteId", $coroner, NULL, "blue"),

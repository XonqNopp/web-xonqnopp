...
if(isset($_POST["title"])) {
	if(isset($_POST["erase"])) {
		// Erase entry
		$id = $_POST["id"];
		$page->dbHelper->idManage("DELETE FROM `{$page->dbHelper->dbName}` . `books` WHERE `books` . `id` = ? LIMIT 1;", $id);
		...
		$query = "";
		if($id > 0) {
			$query = $page->dbHelper->queryPrepare("UPDATE `{$page->dbHelper->dbName}` . `books` SET `isbn` = ?, `title` = ?, `author` = ?, `serie` = ?, `number` = ?, `publisher` = ?, `date` = ?, `language` = ?, `category` = ?, `summary` = ? WHERE `books` . `id` = ? LIMIT 1;");
			$query->bind_param("ssssssssssi", $isbn, $title, $author, $serie, $number, $publisher, $date, $language, $category, $summary, $id);

		} else {
			$query = $page->dbHelper->queryPrepare("INSERT INTO `{$page->dbHelper->dbName}` . `books` (`id`, `isbn`, `title`, `author`, `serie`, `number`, `publisher`, `date`, `language`, `category`, `summary`) VALUES(NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
...
if(isset($_GET["isbn"])) {
	...
	$isbn = $_GET["isbn"];
	$checkisbn = $page->dbHelper->idManage("SELECT * FROM `books` WHERE `isbn` = ?", $isbn);
...
if($id > 0) {
	// Fetch infos from DB
	$query = $page->dbHelper->idManage("SELECT * FROM `{$page->dbHelper->dbName}` . `books` WHERE `books` . `id` = ? LIMIT 1;", $id);
	$query->store_result();
	if($query->num_rows == 0) {
		$query->close();
		exit("Error bad id");
...
// Title
$titleAttr = FieldAttributes(true);
$titleAttr->size = 50;

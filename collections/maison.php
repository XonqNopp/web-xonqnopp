...
//$page->cssHelper->dirUpWing();
    ...
    $hypothecaire = "hypoth&eacute;caire";
    $ideal = "id&eacute;al";
    $impot = "imp&ocirc;t";
...
function liliPlus($content) {
    global $page;
    return $page->bodyBuilder->lili($content, "plus");
}


function liliMinus($content) {
    global $page;
    return $page->bodyBuilder->lili($content, "minus");
}
...
//
    ...
    $body .= "<ul>\n";
    $body .= $page->bodyBuilder->lili("Les $interets de l'$hyp en se basant sur un taux de 5% du $credit par an");
    $body .= $page->bodyBuilder->lili("L'amortissement pour ramener l'$hyp au 2/3 de la valeur du bien $apres 15 ans");
    $body .= $page->bodyBuilder->lili("L'entretien du bien, en comptant 1% de la valeur du bien par an");
    $body .= "</ul>\n";
    ...
    $body .= "<ul>\n";
    $body .= $page->bodyBuilder->lili("cash");
    $body .= $page->bodyBuilder->lili("terrain");
    $body .= $page->bodyBuilder->lili("2e piliers (max 10% de la valeur du bien)");
    $body .= $page->bodyBuilder->lili("3e piliers");
    $body .= $page->bodyBuilder->lili("des avances sur h&eacute;ritages");
    $body .= $page->bodyBuilder->lili("des futurs travaux: $negocier une baisse des fonds propres en promettant d'effectuer des travaux");
    $body .= "</ul>\n";
...
//
    ...
    $body .= $page->bodyBuilder->titleAnchor("Recherche du financement");

        // 2e pilier
        $body .= $page->bodyBuilder->titleAnchor($p2, 3);
        $body .= "<div>\n";

        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili(
            "On peut retirer le $p2 jusqu'$aa 50 ans."
            . " Pass&eacute; ce cap, on peut sortir le maximum de l'$etat qu'il avait $aa 50 ans ou la moiti&eacute; actuelle."
        );
        $body .= $page->bodyBuilder->lili("On peut utiliser son $p2 au maximum jusqu'$aa 3 ans avant l'&acirc;ge de la retraite <b>selon le r&egrave;glement de la caisse de pension</b>.");
        $body .= $page->bodyBuilder->lili("Les pr&eacute;l&egrave;vements dans le $p2 sont sous les conditions de retirer au minimum 20$kchf, un retrait par tranche de 5 ans");
        $body .= $page->bodyBuilder->lili("On n'a pas le droit de pr&eacute;lever dans le $p2 si l'on touche une rente AI.");
        $body .= $page->bodyBuilder->lili(
            "La caisse de pension fait $generalement le versement dans les 6 mois,"
            . " mais cela peut aller jusqu'$aa 24 mois si elle a des {$probleme}s de liquidit&eacute;s."
        );
        $body .= $page->bodyBuilder->lili(
            "Il est beaucoup plus rentable de mettre le $p2 en gage (nantissement) "
            . "$plutot que d'en retirer une partie, mais ce n'est pas accept&eacute; partout."
        );
        $body .= $page->bodyBuilder->lili(
            "Le versement du $p2 est soumis aux {$impot}s! Penser $aa en tenir compte dans le calcul des fonds propres."
            . " Note: l'$impot est restitu&eacute; au remboursement <b>seulement si demand&eacute;</b> dans les 3 ans suivant le remboursement."
        );
        $body .= $page->bodyBuilder->lili(
            "Demander $aa la caisse de pension un comparatif des prestations avant-$apres le retrait."
        ...
        $body .= $page->bodyBuilder->lili("On peut rembourser jusqu'$aa 3 ans avant la retraite, chaque remboursement au minimum 20$kchf.");
        $body .= $page->bodyBuilder->lili(
            "Il est parfois possible de conclure une assurance compl&eacute;mentaire"
            . " $aupres de la caisse de pension pour &eacute;viter une r&eacute;duction de la rente de retraite."
        );
        $body .= "</ul>\n";
    ...
    //
        ...
        $body .= "<li><b>Autres:</b>\n";
        $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("PostFinance (UBS)");
            $body .= $page->bodyBuilder->lili("HypothekenZentrum (VZ): sp&eacute;cialis&eacute; dans les {$hyp}s, frais administratifs avantageux");
            $body .= $page->bodyBuilder->lili("homegate.ch");
            $body .= $page->bodyBuilder->lili("HausEigentuemerVerband");
            $body .= $page->bodyBuilder->lili("Certaines casses de pension (seulement 10% proposent des {$hyp}s): payes les $interets financent notre retraite :-)");
        $body .= "</ul>\n";
        $body .= "</li>\n";
    ...
    //
        // Types d'hyp
        $body .= $page->bodyBuilder->titleAnchor("Types d'$hyp", 3);
        $body .= "<div>\n";
        ...
        $body .= $page->bodyBuilder->lili("Combi/mixte: un mix de taux fixe et de taux variable.");
        ...
        $body .= $page->bodyBuilder->lili("<b>Libor Cap/Strike:</b> avec plafond mais plus cher. Pas rentable pour moins de 5 ans.");

        $body .= "<li><b>Variantes de Libor Cap:</b>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("BCV Benefit (6 mois)");
        $body .= $page->bodyBuilder->lili("UBS Libor Cap Warrants");
        $body .= $page->bodyBuilder->lili("Cr&eacute;dit Suisse Flex: max + min");
        $body .= "</ul>\n";
        $body .= "</li>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("start/USB FirstHome: rabais sur les taux pour une partie du $credit si c'est le premier logement.");
        $body .= $page->bodyBuilder->lili("Eco/Minergie");
        $body .= $page->bodyBuilder->lili("Famille");
        $body .= "</ul>\n";
    ...
    //
        // Amortissement
        $body .= $page->bodyBuilder->titleAnchor("Amortissement", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("$deductionsF des $interets sur le revenu");
        $body .= $page->bodyBuilder->lili("$deductionsF de la dette sur la fortune");
        $body .= $page->bodyBuilder->lili("$deductionsF des primes d'amortissements sur le revenu");
        $body .= $page->bodyBuilder->lili("charges initiales plus basses");
        $body .= $page->bodyBuilder->lili("couvert en cas de $deces");
        $body .= $page->bodyBuilder->lili("exon&eacute;ration des primes en cas d'incapacit&eacute; de travail");
        $body .= "</ul>\n";
    ...
    //
        // Strategie hyp
        $body .= $page->bodyBuilder->titleAnchor("Strat&eacute;gie $hypothecaire", 3);
    ...
    //
        // Negocier hyp
        $body .= $page->bodyBuilder->titleAnchor("N&eacute;gocier l'$hyp", 3);

            // Documents
            $body .= $page->bodyBuilder->titleAnchor("Documents $aa r&eacute;unir", 4);
            $body .= "<div>\n";

            $body .= "<p>Informations personnelles:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("formulaire de demande de $credit $hypothecaire");
            $body .= $page->bodyBuilder->lili("derni&egrave;re d&eacute;claration fiscale");
            $body .= $page->bodyBuilder->lili("copie des contrats de leasing et/ou de pr&ecirc;ts personnels");
            $body .= $page->bodyBuilder->lili("convention de divorce");
            $body .= $page->bodyBuilder->lili("certificats de salaire et fiches de paye");
            $body .= $page->bodyBuilder->lili("extrait du registre des poursuites sur 3 ans (datant de 3 mois au maximum)");
            $body .= $page->bodyBuilder->lili("{$piece}s d'identi&eacute;, livret de famille");
            $body .= $page->bodyBuilder->lili("certificat de $prevoyance ($p2)");
            $body .= $page->bodyBuilder->lili("liste de tous les fonds propres");
            $body .= $page->bodyBuilder->lili("d&eacute;claration stipulant que vous habiterez vous-m&ecirc;me le logement (si utilisation 2e et/ou $p3)");
            $body .= $page->bodyBuilder->lili("extrait du $RF pour d'autres biens vous appartenant");
            $body .= $page->bodyBuilder->lili("justificatif des avoirs du $p3");
            $body .= "</ul>\n";

            $body .= "<p>Informations sur le bien existant:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("extrait du $RF ou copie du projet de contrat d'acquisition");
            $body .= $page->bodyBuilder->lili("plan de situation ou plan du cadastre");
            $body .= $page->bodyBuilder->lili("polices d'assurances de l'immeuble");
            $body .= $page->bodyBuilder->lili("plans {$detaille}s de construction 1:100 ou 1:50 (si $aa disposition)");
            $body .= $page->bodyBuilder->lili("plaquette de vente, photos");
            $body .= $page->bodyBuilder->lili("$etat locatif");
            $body .= $page->bodyBuilder->lili("descriptif de la construction: ann&eacute;e, volume SIA ou assurance incendie");
            $body .= $page->bodyBuilder->lili("description des $renovations/transformations");
            $body .= $page->bodyBuilder->lili("devis et contrats d'entreprises g&eacute;n&eacute;rales");
            $body .= $page->bodyBuilder->lili("contrat de vente authentique (maison, terrain)");
            $body .= $page->bodyBuilder->lili("photos actuelles du bien");
            $body .= $page->bodyBuilder->lili("permis de construre (si disponible)");
            $body .= "</ul>\n";

            $body .= "<p>Informations sur objet sur plan:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("catalogue du promoteur");
            $body .= $page->bodyBuilder->lili("plans {$detaille}s de l'objet 1:100 ou 1:50");
            $body .= $page->bodyBuilder->lili("descriptif de la construction");
            $body .= $page->bodyBuilder->lili("projet de contrat d'entreprise g&eacute;n&eacute;rale");
            $body .= "</ul>\n";

            $body .= "<p>Informations sur la PPE:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("surfaces habitables, terrasse et/ou jardin");
            $body .= $page->bodyBuilder->lili("situation du fonds de $renovations");
            $body .= $page->bodyBuilder->lili("expertise (si disponible)");
            $body .= $page->bodyBuilder->lili("acte constitutif de la PPE");
            $body .= $page->bodyBuilder->lili("r&egrave;glement d'utilisation et d'administration de la PPE");
            $body .= $page->bodyBuilder->lili("bilan et comptes d'exploitation de la PPE");
            $body .= "</ul>\n";

            $body .= "<p>Informations sur la nouvelle construction:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("acte de vente du terrain");
            $body .= $page->bodyBuilder->lili("plans {$detaille}s du projet de construction 1:100 ou 1:50");
            $body .= $page->bodyBuilder->lili("descriptif $detaille de la construction");
            $body .= $page->bodyBuilder->lili("plan financier $detaille (si possible bas&eacute; sur le volume SIA)");
            $body .= $page->bodyBuilder->lili("liste des artisans participant au chantier (si disponible)");
            $body .= $page->bodyBuilder->lili("permis de construire");
            $body .= "</ul>\n";

            $body .= "<p>Informations pour transformations/$renovations:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("plans {$detaille}s du projet 1:100 ou 1:50");
            $body .= $page->bodyBuilder->lili("liste des artisans participant au chantier (si disponible)");
            $body .= $page->bodyBuilder->lili("permis de construire (si n&eacute;cessaire pour le projet)");
            $body .= $page->bodyBuilder->lili("devis $detaille, si possible bas&eacute; sur les offres");
            $body .= "</ul>\n";

            $body .= "</div>\n";
        //
            // Tour horizon
            $body .= $page->bodyBuilder->titleAnchor("Tour d'horizon", 4);
            $body .= "<div>\n";
            ...
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("une grande banque: UBS, Credit Suisse...");
            $body .= $page->bodyBuilder->lili("une banque de proximit&eacute;: Raiffeisen...");
            $body .= $page->bodyBuilder->lili("la banque cantonale du canton et d'un canton voisin");
            $body .= $page->bodyBuilder->lili("une compagnie d'assurance: Swisslife...");
            $body .= "</ul>\n";
        ...
        //
            // Concurrence
            $body .= $page->bodyBuilder->titleAnchor("Faire jouer la concurrence", 4);
        ...
        //
            // Entretien
            $body .= $page->bodyBuilder->titleAnchor("Entretien personnel", 4);
            $body .= "<div>\n";
            $body .= "<p>Avec les 2e offres, en choisir 3-5 et prendre RDV pour un entretien personnel.</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("Pr&eacute;parer une liste de questions");
            $body .= $page->bodyBuilder->lili("Mentionner la possibilit&eacute; de migrer les comptes courant, &eacute;pargnes, 3a");
            $body .= $page->bodyBuilder->lili("Demander une confirmation &eacute;crite de la derni&egrave;re proposition avec toutes les conditions");
            $body .= "</ul>\n";
            $body .= "</div>\n";
        //
            // Courtier
            $body .= $page->bodyBuilder->titleAnchor("Courtier en $hyp", 4);
        ...
        //
            // Vocabulaire
            $body .= $page->bodyBuilder->titleAnchor("Vocabulaire", 4);
            $body .= "<p>Quand la banque accorde le $pret, le notaire cr&eacute;e une <b>c&eacute;dule $hypothecaire</b> exig&eacute;e comme garantie par la banque.\n";
    ...
    //
        // Renouveler
        $body .= $page->bodyBuilder->titleAnchor("Renouveler l'$hyp", 3);
        $body .= "<div>\n";
        $body .= "<p>Quand arrive l'$echeance et qu'il faut renouveler l'$hyp, il s'agit de:</p>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("Etudier l'amortissement");
        ...
        $body .= $page->bodyBuilder->lili("Faire jouer la concurrence: mentionner le meilleur taux du voisin pour essayer d'avoir un geste");
        $body .= $page->bodyBuilder->lili("Choisir un long taux fixe seulement si on est certain de ne pas d&eacute;m&eacute;nager ni amortir");
    ...
    //
        // Credit construction
        $body .= $page->bodyBuilder->titleAnchor("Cr&eacute;dit de construction", 3);
...
//
    // Acheter quoi?
    $body .= $page->bodyBuilder->titleAnchor("Acheter quoi?");

        // Logement ideal
        $body .= $page->bodyBuilder->titleAnchor("Logement $ideal", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("ville, campagne, banlieue, montagne?");
        $body .= $page->bodyBuilder->lili("voiture, transports publics?");
        $body .= $page->bodyBuilder->lili("temps de trajet maximum pour aller au travail?");
        $body .= $page->bodyBuilder->lili("infrastructures: cr&egrave;che, &eacute;cole, sport, culture, commerces, m&eacute;decin, poste, banque?");
        $body .= $page->bodyBuilder->lili("surface du terrain d&eacute;sir&eacute;e: jardin, jeux, garage?");
        $body .= $page->bodyBuilder->lili("voisinage: &acirc;ge, niveau social, avec/sans enfants?");
        $body .= $page->bodyBuilder->lili("environnement: Soleil, climat, nature, routes, agricole, r&eacute;sidentiel?");
        $body .= $page->bodyBuilder->lili("pollution sonore");
        $body .= $page->bodyBuilder->lili("{$impot}s");
        $body .= "</ul>\n";

        $body .= "<p><b>Espace:</b> (voir aussi " . getLink("http://hausinfo.ch") . " et " . getLink("http://ubs.com") . "</p>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("combien d'habitants $aa court et long terme? Des animaux?");
        $body .= $page->bodyBuilder->lili("travail $aa domicile dans une $piece s&eacute;par&eacute;e (th&eacute;rapeute)?");
        $body .= $page->bodyBuilder->lili("hobby n&eacute;cessitant un espace sp&eacute;cial int-/ext&eacute;rieur (bricolage, musique...)?");
        $body .= $page->bodyBuilder->lili("divorc&eacute; accueillant p&eacute;riodiquement des enfants?");
        $body .= $page->bodyBuilder->lili("chambre d'amis?");
        $body .= $page->bodyBuilder->lili("chacun son espace/sa chambre?");
        $body .= "</ul>\n";

        $body .= "<p><b>Habitat:</b></p>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("quelle forme? (voir suite)");
        $body .= $page->bodyBuilder->lili("jardin/terrasse, balcon?");
        $body .= $page->bodyBuilder->lili("moderne, ancien, rustique, design, ecolo?");
        $body .= $page->bodyBuilder->lili("{$piece}s: spacieuses, faciles $aa transformer plus tard, plain-pied?");
        $body .= $page->bodyBuilder->lili("aussi pour la retraite ($mobiliteReduite)?");
        $body .= "</ul>\n";

        $body .= "</div>\n";
    //
        // Formes d'habitat
        $body .= $page->bodyBuilder->titleAnchor("Formes d'habitat", 3);
        ...
        $body .= $page->butler->rowOpen() . $page->butler->headerCell("Habitat") . $page->butler->headerCell("Nombres de faces") . $page->butler->rowClose();
        $body .= $page->butler->row(array("6 villas individuelles", "30"));
        $body .= $page->butler->row(array("6 villas mitoyennes", "24"));
        $body .= $page->butler->row(array("6 villas contigu&euml;es", "20"));
        $body .= $page->butler->row(array("immeuble de 6 appartements", "19"));
        $body .= $page->butler->tableClose();
        $body .= "</div>\n";

            // Individuelle
            $body .= $page->bodyBuilder->titleAnchor("Maison individuelle", 4);
            $body .= "<div>\n";
            $body .= "<ul>\n";
        ...
        //
            // Bungalow
            $body .= $page->bodyBuilder->titleAnchor("Bungalow (maison sur 1 $etage)", 4);
            $body .= "<p>Comme pour une maison individuelle mais avec plus de terrain et plus cher.</p>\n";
        //
            // Individuelle en lotissement
            $body .= $page->bodyBuilder->titleAnchor("Maison individuelle en lotissement", 4);
            $body .= "<div>\n";
            $body .= "<ul>\n";
        ...
        //
            // Mitoyenne
            $body .= $page->bodyBuilder->titleAnchor("Maison mitoyenne/jumelle", 4);
            $body .= "<div>\n";
            $body .= "<ul>\n";
        ...
        //
            // En terrasses
            $body .= $page->bodyBuilder->titleAnchor("Maison en terrasses", 4);
            $body .= "<div>\n";
            $body .= "<ul>\n";
        ...
        //
            // PPE
            $body .= $page->bodyBuilder->titleAnchor("Appartement en PPE", 4);
            $body .= "<div>\n";
            $body .= "<ul>\n";
            ...
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("situation calme et ensoleill&eacute;e");
            $body .= $page->bodyBuilder->lili("{$piece}s lumineuses");
            $body .= $page->bodyBuilder->lili("agencement pratique et faciles d'entretien");
            $body .= $page->bodyBuilder->lili("grandes surfaces");
            $body .= $page->bodyBuilder->lili("salon avec balcon/terrasse");
            $body .= $page->bodyBuilder->lili("cuisine et salles de bains modernes et pratiques");
            $body .= $page->bodyBuilder->lili("suffisamment de rangements (armoires encastr&eacute;es, cagibi)");
            $body .= $page->bodyBuilder->lili("mode de construction de qualit&eacute;");
            $body .= $page->bodyBuilder->lili("isolation phonique sup&eacute;rieure - attention: locatif transform&eacute; en PPE souvent pas bon, le prix n'est pas gage de confort");
            $body .= $page->bodyBuilder->lili("immeuble joli");
            $body .= "</ul>\n";

            $body .= "</div>\n";
    //
        // Neuf/occasion
        $body .= $page->bodyBuilder->titleAnchor("Neuf ou occasion", 3);

            // Occasion
            $body .= $page->bodyBuilder->titleAnchor("Occasion", 4);
            ...
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("<b>faiblement agglom&eacute;r&eacute;:</b> risque permanent (choc, vibrations)");
            $body .= $page->bodyBuilder->lili("<b>fortement agglom&eacute;r&eacute;:</b> risque si travaux (cass&eacute;, perc&eacute;...)");
            $body .= "</ul>\n";
            ...
            $body .= $page->bodyBuilder->lili("peintures au plomb");
            $body .= $page->bodyBuilder->lili("radon " . getLink("http://ch-radon.ch"));
            ...
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("l'objet est-il grev&eacute; d'{$hyp}s?");
            $body .= $page->bodyBuilder->lili("existe-t-il des servitudes/droits de passage?");
            $body .= $page->bodyBuilder->lili("est-il class&eacute; monument historique? (transformations limit&eacute;es)");
            $body .= $page->bodyBuilder->lili("quelqu'un est-il acheteur prioritaire (droit de pr&eacute;emption) ou utilisation $aa vie (usufruit) du bien?");
            $body .= "</ul>\n";
        ...
        //
            // Neuf
            $body .= $page->bodyBuilder->titleAnchor("Neuf", 4);
            $body .= "<div>\n";
    ...
    //
        // Mobilite reduite
        $body .= $page->bodyBuilder->titleAnchor("Mobilit&eacute; r&eacute;duite", 3);
        $body .= "<div>\n";
        $body .= "<p>Sachant qu'on peut chacun $etre $aa $mobiliteReduite $aa un moment de sa vie, il est bon d'anticiper:</p>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("acc&eacute;s partout en fauteuil roulant (garage, buanderie, cave, galetas...), {$ideal}ement un seul niveau, sinon ascenceur $aa disposition");
        $body .= $page->bodyBuilder->lili("quartier avec commerces, m&eacute;decin, transport publics, restaurant");
        $body .= $page->bodyBuilder->lili("douche, si baignoire pr&eacute;voir des poign&eacute;es");
        $body .= $page->bodyBuilder->lili("si possible des stores $plutot que des volets");
        $body .= $page->bodyBuilder->lili("moquette (antid&eacute;rapante) dans les escaliers");
        $body .= $page->bodyBuilder->lili("lampes claires, interrupteurs pr&egrave;s des portes et du lit");
        $body .= "</ul>\n";
        $body .= "</div>\n";
    //
        // Minimum exigible
        $body .= $page->bodyBuilder->titleAnchor("Minimum exigible", 3);
        $body .= "<div>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("un $batiment neuf devrait $etre minergie");
        $body .= $page->bodyBuilder->lili("20-25% de surface en plus qu'un locatif: 100m2 pour 3p, 125 pour 4, 150 pour 5)");
        $body .= $page->bodyBuilder->lili("grand salon, grande cuisine ouverte (30m2), 2 salles de bains, 14m2 par chambre quasi carr&eacute;e");
        $body .= $page->bodyBuilder->lili("terrasse/balcon au moins 1.5m de large");
        $body .= $page->bodyBuilder->lili("escaliers lumineux et chauff&eacute;s, toutes les {$piece}s accessibles avec l'ascenceur");
        $body .= "</ul>\n";
        $body .= "</div>\n";

            $body .= $page->bodyBuilder->titleAnchor("Equipements", 4);
            $body .= "<div>\n";

            $body .= "<p>Cuisine:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("plan de travail en pierre ou en inox");
            $body .= $page->bodyBuilder->lili("armoires coulissantes");
            $body .= $page->bodyBuilder->lili("&eacute;clairages int&eacute;gr&eacute;s aux surfaces de travail");
            $body .= $page->bodyBuilder->lili("lpan de cuisson vitroc&eacute;ramique ou induction");
            $body .= $page->bodyBuilder->lili("lave-vaisselle classe &eacute;nerg&eacute;tique A");
            $body .= $page->bodyBuilder->lili("four $aa hauteur des yeux avec fonction vapeur");
            $body .= $page->bodyBuilder->lili("frigo avec compartiment cong&eacute;lation ou cong&eacute;lateur s&eacute;par&eacute;");
            $body .= "</ul>\n";

            $body .= "<p>Salles de bains:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("carrelage du sol au plafond sur tous les murs");
            $body .= $page->bodyBuilder->lili("grande armoire pharmacie");
            $body .= $page->bodyBuilder->lili("cabine de douche ou paroi de baignoire en alu et verre, surface minimum 0.8m2");
            $body .= $page->bodyBuilder->lili("place de stockage pour produits de nettoyage et linges");
            $body .= $page->bodyBuilder->lili("baignoire de minimum 1.80m de long ou d'angle");
            $body .= "</ul>\n";

            $body .= "<p>Installations &eacute;lectriques:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("prises TV-radio dans toutes les {$piece}s");
            $body .= $page->bodyBuilder->lili("interrupteurs $aa distance pour les lampes $aa pied");
            $body .= $page->bodyBuilder->lili("bo&icirc;tier de c&acirc;blage universel (informatique, t&eacute;l&eacute;phone)");
            $body .= $page->bodyBuilder->lili("actionnement de prises, lampes, stores etc. par un syst&egrave;me de bus de commande");
            $body .= "</ul>\n";

            $body .= "<p>Sols:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("parquet ou carrelage, carrelage pour cuisine et salles de bains");
            $body .= "</ul>\n";

            $body .= "<p>Chauffage/ventilation:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("chauffage au sol ou radiateurs dans toutes les {$piece}s");
            $body .= $page->bodyBuilder->lili("radiateur s&egrave;che-linge dans les salles de bains");
            $body .= $page->bodyBuilder->lili("ventilation contr&ocirc;l&eacute;e avec r&eacute;cup&eacute;rateur de chaleur");
            $body .= "</ul>\n";

            $body .= "<p>Fen&ecirc;tres:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("doubles voire triples vitrages isolants et antibruits pour les lieux bruyants");
            $body .= $page->bodyBuilder->lili("stores $aa lamelles int&eacute;gr&eacute;s");
            $body .= "</ul>\n";

            $body .= "<p>Buanderie:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("lave-linge et s&eacute;choir individuels dans l'appartement ou au sous-sol");
            $body .= "</ul>\n";

            $body .= "<p>Cave:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("cave individuelle ferm&eacute;e avec des parois (pas des lattes en bois)");
            $body .= "</ul>\n";

            $body .= "<p>Meubles int&eacute;gr&eacute;s:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("armoires dans les chambres $aa coucher");
            $body .= $page->bodyBuilder->lili("vestiaire $aa l'entr&eacute;e");
            $body .= "</ul>\n";

            $body .= "<p>Garage:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("<b>immeubles:</b> une place au garage avec armoire fermant $aa cl&eacute; (pneus, porte-bagages...) et une place ext&eacute;rieure");
            $body .= $page->bodyBuilder->lili("<b>maison:</b> un box ferm&eacute; pour 2 voitures ou garage souterrain");
            $body .= "</ul>\n";

            $body .= "</div>\n";
    //
        // Pompes a chaleur
        $body .= $page->bodyBuilder->titleAnchor("Pompes $aa chaleur", 3);
        $body .= "<div>\n";
        $body .= "<p>Il en existe plusieurs sortes:</p>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("<b>air/eau:</b> 30$kchf (40$kchf avec solaire) $ideal pour le chauffage au sol, facile $aa installer, mais utilise un gros ventilateur bruyant pour soi et le voisinage.");
        $body .= $page->bodyBuilder->lili("<b>eau/eau:</b> 50$kchf et besoin d'un plan d'eau ou une nappe phr&eacute;atique ainsi que des autorisations difficiles $aa obtenir!!");
        $body .= $page->bodyBuilder->lili("<b>sol/eau:</b> 45$kchf seulement si le terrain souterrain est OK, avec une autorisation cantonale, une mise $aa l'enqu&ecirc;te. Pour l'installation il faut une foresue de 20 tonnes qui doit pouvoir acc&eacute;der, le forage $coute 75.-/m et il faut creuser au moins 180m.");
        $body .= "</ul>\n";
        $body .= "</div>\n";
    //
        // Ecologie
        $body .= $page->bodyBuilder->titleAnchor("Ecologie", 3);
        $body .= "<div>\n";
        ...
        $body .= "Le chauffage est la plus grand source de consommation d'$energie.\n";
...
//
    // Recherche
    $body .= $page->bodyBuilder->titleAnchor("Recherche de l'objet");
    ...
    $body .= "Les assurances, les banques et les caisses de pension vendent aussi des biens immobiliers.</p>\n";
        ...
        // Dechifrer
        $body .= $page->bodyBuilder->titleAnchor("D&eacute;chifrer", 3);
        $body .= "<div>\n";
        $body .= "<ul>\n";
        ...
        $body .= $page->bodyBuilder->lili("Equipements de qualit&eacute;: $aa voir sur place, possible qualit&eacute; minimum.");
        $body .= $page->bodyBuilder->lili("Images, animations, photos: se m&eacute;fier de ce qui n'est pas montr&eacute;.");
        $body .= $page->bodyBuilder->lili("Si le bien est $aa vendre depuis longtemps, quelque chose cloche (prix, $etat, situation, voisins...).");

        $body .= "</ul>\n";
        $body .= "</div>\n";
    //
        // Aide
        $body .= $page->bodyBuilder->titleAnchor("Aide", 3);
        $body .= "<div>\n";
        $body .= "<ul>\n";
        ...
        $body .= $page->bodyBuilder->lili("Agent immobilier: 200.-/h");
    ...
    //
        ...
        $body .= $page->bodyBuilder->lili("Visiter et examiner avec un professionel, car l'ench&egrave;re exclu le droit en cas de d&eacute;faut d&eacute;couverts ensuite.");
        $body .= $page->bodyBuilder->lili("Examiner le $RF: &eacute;ventuels charges ($hyp), droits (pr&eacute;emption, usufruit), obligations (servitudes, passage).");
        $body .= $page->bodyBuilder->lili("R&eacute;gler le financement $aa l'avance (la banque du vendeur risque de refuser, en choisir une autre).");
        $body .= $page->bodyBuilder->lili("V&eacute;rifier le prix estim&eacute;, si doutes faire faire une estimation.");
        $body .= $page->bodyBuilder->lili("Fixer une limite (fonds propres et $hyp) $aa l'avance <b>en accord</b> avec le banquier.");
        $body .= $page->bodyBuilder->lili("Pr&eacute;parer un ch&egrave;que avec l'accompte (10-20% du prix estim&eacute;), exig&eacute; $apres le coup de marteau.");
    ...
    //
        // Prix
        $body .= $page->bodyBuilder->titleAnchor("Prix", 3);
        ...
        $body .= "<p>Le principal facteur d&eacute;terminant le prix est la situation: voisinage, $equipements, acc&egrave;s, vue, Soleil, bruit, {$impot}s (d&eacute;chets, eau...).</p>\n";

            // Experts
            $body .= $page->bodyBuilder->titleAnchor("Experts", 4);
            $body .= "<div>\n";
        ...
        //
            // Negocier
            $body .= $page->bodyBuilder->titleAnchor("N&eacute;gocier", 4);
            $body .= "<div>\n";
        ...
        //
            // Visite
            $body .= $page->bodyBuilder->titleAnchor("Visite", 4);
            $body .= "<div>\n";
            ...
            $body .= "<p>\"Am&eacute;nagements ext&eacute;rieurs\" pour objet neuf: $aa clarifier en d&eacute;tail car les promoteurs &eacute;conomisent souvent.</p>\n";

            $body .= "</div>\n";
//
    // Construire: preparation
    $body .= $page->bodyBuilder->titleAnchor("Construire: pr&eacute;paration");
    ...
    $body .= "Il est possible de partager les frais de chantier et de raccordement si le terrain voisin construit en $meme temps.</p>\n";
        ...
        // Terrain
        $body .= $page->bodyBuilder->titleAnchor("Terrain", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("Demander $aa la commune: elle sait avant le public, et poss&egrave;de aussi des terrains");
        $body .= $page->bodyBuilder->lili("Aller voir s'il y a des pancartes sur place, des annonces dans les magasin...");
        ...
        $body .= "le terrain correspond-il au prix du $marche de la r&eacute;gion? (se renseigner $aupres de fisc, banques r&eacute;gionales/cantonales, courtier, voisins)\n";
    ...
    //
        ...
        $body .= $page->bodyBuilder->titleAnchor("Servitudes fonci&egrave;res", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("canalisations");
        $body .= $page->bodyBuilder->lili("construction (droit de superficie, voir dessous)");
        $body .= $page->bodyBuilder->lili("empi&egrave;tement, restrictions du droit $aa b&acirc;tir");
        $body .= $page->bodyBuilder->lili("droit de passage (pied, v&eacute;hicule), parcage");
        $body .= $page->bodyBuilder->lili("plantations (tailler les haies), cl&ocirc;tures (repeindre)");
        $body .= $page->bodyBuilder->lili("divers: usufruit, usage ext&eacute;rieur, abri PC...");
        $body .= "</ul>\n";
    ...
    //
        ...
        $body .= $page->bodyBuilder->titleAnchor("R&egrave;glement de construction (communal)", 3);
    ...
    //
        ...
        $body .= $page->bodyBuilder->titleAnchor("Influences n&eacute;gatives", 3);
    ...
    //
        // Concevoir sa maison
        $body .= $page->bodyBuilder->titleAnchor("Concevoir sa maison", 3);
        $body .= "<div>\n";
        ...
        $body .= $page->bodyBuilder->lili("<b>Maison d'architecte:</b> sur mesure, tout personnalisable, unique. Co&ucirc;te cher et prend tu temps pour $etre r&eacute;alis&eacute;.");
        ...
        $body .= $page->bodyBuilder->lili("<b>Maison syst&egrave;me:</b> plans existants mais construite sur place par des artisans; comme la maison type.");
    ...
    //
        // Plans
        $body .= $page->bodyBuilder->titleAnchor("Plans", 3);
        $body .= "<div>\n";
        $body .= "<ul>\n";
        ...
        $body .= $page->bodyBuilder->lili("<b>Vestibule:</b> plaque tournante. Bien &eacute;clair&eacute; et spacieux.");
        $body .= $page->bodyBuilder->lili("<b>Salon:</b> le plus au Sud possible (moins de Soleil en $ete et plus en hiver). Acc&egrave;s jardin/terrasse.");
        $body .= $page->bodyBuilder->lili("<b>Salle $aa manger:</b> bien reli&eacute;e $aa cuisine et salon.");
        $body .= $page->bodyBuilder->lili("<b>Cuisine:</b> proche de l-entr&eacute;e, du salon, et la salle $aa manger, des chambres d'enfatns et avec vus sur la place de jeux.");
        $body .= $page->bodyBuilder->lili("<b>Buanderie:</b> proche de cuisine et salles de bains, si possible $aa la cave ou $aa l'&eacute;tage.");
        ...
        $body .= $page->bodyBuilder->lili("<b>Salles de bains:</b> $cote dortoir, pas coll&eacute;es (murs) aux chambres mais reli&eacute;es par un couloir.");
        $body .= $page->bodyBuilder->lili("<b>WC visiteurs:</b> proches de l'entr&eacute;e, &eacute;loign&eacute;s de la cuisine et du salon.");
        $body .= $page->bodyBuilder->lili("<b>Chambres d'amis:</b> acc&egrave;s $aa l'entr&eacute;e, &eacute;ventuellement petite salle de bains.");
        $body .= $page->bodyBuilder->lili("<b>Bureau:</b> acc&egrave;s salon et vestibule.");
        $body .= $page->bodyBuilder->lili("<b>Rangements:</b> Nord pour pas chauffer, a&eacute;r&eacute;s.");
        $body .= $page->bodyBuilder->lili("<b>Cave:</b> attention $aa l'humidit&eacute;.");
...
//
    ...
    $body .= $page->bodyBuilder->titleAnchor("Construire: ex&eacute;cution");

        // Avec un architecte
        $body .= $page->bodyBuilder->titleAnchor("Avec un architecte", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("ses maisons r&eacute;alis&eacute;es nous plaisent (esth&eacute;tique, qualit&eacute;)");
        $body .= $page->bodyBuilder->lili("il tient les d&eacute;lais et le budget");
        $body .= $page->bodyBuilder->lili("ses maisons s'int&egrave;grent dans l'environnement");
        $body .= $page->bodyBuilder->lili("il est int&eacute;ress&eacute; $aa la construction &eacute;cologique, id&eacute;alement avec de l-exp&eacute;rience");
        $body .= $page->bodyBuilder->lili("il est situ&eacute; proche du futur chantier (plus il est loin, plus il y aura de frais)");
        $body .= $page->bodyBuilder->lili("il conna&icirc;t le lieu (et ses r&egrave;glements)");
        $body .= $page->bodyBuilder->lili("il est ouvert aux id&eacute;es nouvelles et coop&eacute;ratif");
        $body .= "</ul>\n";
        ...
        $body .= "</div>\n";
            ...
            $body .= $page->bodyBuilder->titleAnchor("Contrats de l'architecte", 4);
        ...
        //
            ...
            $body .= $page->bodyBuilder->titleAnchor("Honoraires de l'architecte", 4);
            $body .= "<div>\n";

            $body .= "<p>SIA 102</p>\n";

            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("tarif-temps: temps n&eacute;cessaire $aa la r&eacute;alisation. Tarif horaire SIA en fonction de l'exp&eacute;rience.");
            $body .= $page->bodyBuilder->lili("tarif-co&ucirc;t/tarif-volume: en fonction du montant des travaux, de la difficult&eacute; (proportionnel au prix de la maison)");
        ...
        //
            // Devis
            $body .= $page->bodyBuilder->titleAnchor("Devis", 4);
            $body .= "<div>\n";

            $body .= "<p>Ce n'est qu'une &eacute;valuation! Pr&eacute;voir des marges:</p>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("avant-projet: +/-15%");
            $body .= $page->bodyBuilder->lili("projet: +/-10%");
            $body .= $page->bodyBuilder->lili("apr&egrave;s l'appel d'offres: +/-5%");
            $body .= "</ul>\n";
    ...
    //
        ...
        $body .= $page->bodyBuilder->titleAnchor("Entreprise g&eacute;n&eacute;rale/totale", 3);
        $body .= "<div>\n";

        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("<b>Entreprise g&eacute;n&eacute;rale:</b> r&eacute;alise l'ensemble des travaux selon les plans.");
        $body .= $page->bodyBuilder->lili("<b>Entreprise totale:</b> comnme g&eacute;n&eacute;rale, et planfification de projet, peut concevoir des maisons d'architecte.");
        $body .= "</ul>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("qualit&eacute;: pr&eacute;f&eacute;rer un membre de ASEG");
        $body .= $page->bodyBuilder->lili("r&eacute;flexion: pas de pr&eacute;cipitation, m&ecirc;me s'il y a des pressions de l'entreprise.");
        $body .= $page->bodyBuilder->lili("solvabnilit&eacute;: ASEG OK, sinon demander Office des Poursuites. Les petites entreprises ne peuvent pas toujours assumer l'impr&eacute;vu.");
        $body .= $page->bodyBuilder->lili("r&eacute;putation: fuir les mauvaises (demander aux amis, banquier...).");
        $body .= $page->bodyBuilder->lili("lamentations: regarder sur les forums, par ex. " . getLink("http://forumconstruire.ch"));
        $body .= $page->bodyBuilder->lili("documents: ASEG peut fournir un contrat-type pour 25.-. Si l'entreprise impose son propre document, mandater un sp&eacute;cialiste (architecte/juriste) pour v&eacute;rifier!");
        $body .= $page->bodyBuilder->lili("lucidit&eacute;: se faire une id&eacute;e des prix. Les prix d'appel augmentent durant le chantier.");
        $body .= "</ul>\n";

        $body .= "<p>Comparer les offres: les meilleurs prix 'oublient' souvent les ext&eacute;rieurs, ou choisissent des mat&eacute;riaux bon $marche de mauvaise qualit&eacute;...</p>";

        $body .= "</div>\n";

            // Contrats de l entrepreneur
            $body .= $page->bodyBuilder->titleAnchor("Contrats de l'entrepreneur", 4);
            $body .= "<div>\n";
            ...
            $body .= $page->bodyBuilder->lili("le chef du chantier doit $etre formellement nomm&eacute;.");
            ...
            $body .= $page->bodyBuilder->lili("le plan de financement qui garantit que si l'entrepreneur fait faillite, les prestations pay&eacute;es seront bien fournies.");
            ...
            $body .= $page->bodyBuilder->lili("la RC de l'entreprise (dommages aux personnes et aux choses, dommages et vices de construction).");
        ...
        //
            // Hypothese legale
            $body .= $page->bodyBuilder->titleAnchor("Hypoth&egrave;se l&eacute;gale", 4);
            $body .= "<div>\n";
            ...
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("r&eacute;serv&eacute;e aux ma&icirc;tres d'&eacute;tats ind&eacute;pendants qui ont directement travaill&eacute; sur le chantier");
            $body .= $page->bodyBuilder->lili("le mat&eacute;riel fourni doit avoir $ete fait sur mesure");
            $body .= $page->bodyBuilder->lili("au maximum 4 ans apr&egrave;s le travail");
            $body .= "</ul>\n";
        ...
        //
            // Honoraires
            $body .= $page->bodyBuilder->titleAnchor("Honoraires de l'entrepreneur", 4);
            $body .= "<div>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("forfait");
            $body .= $page->bodyBuilder->lili("prix total: comme un forfait, mais varie selon l'&eacute;volution des prix");
            $body .= $page->bodyBuilder->lili("prix unitaire: d&eacute;termin&eacute; selon les unit&eacute;s n&eacute;cessaires, fix&eacute; apr&egrave;s les travaux");
            $body .= $page->bodyBuilder->lili("prix effectif: devis approximatif, prix apr&egrave;s les travaux");
            $body .= "</ul>\n";
            $body .= "</div>\n";
    //
        // Architecte vs Entrepreneur
        $body .= $page->bodyBuilder->titleAnchor("Architecte vs Entrepreneur", 3);
        $body .= "<div>\n";
        ...
        $body .= $page->bodyBuilder->lili("<b>Libert&eacute;:</b>parfois aussi avec un entrepreneur, mais souvent les changements de plans sont tr&egrave;s co&ucirc;teux.");
        $body .= $page->bodyBuilder->lili("<b>Choix des mat&eacute;riaux et artisans:</b> pas possible avec un entrepreneur, ce qui fait un risque d'$hyp l&eacute;gale.");
        $body .= $page->bodyBuilder->lili("Le projet est unique, les erreurs sont possibles et il y a de multiples interlocuteurs.");
        $body .= $page->bodyBuilder->lili("Les co&ucirc;ts sont variables.");
        $body .= $page->bodyBuilder->lili("Les d&eacute;lais seront d&eacute;pass&eacute;s.");
        $body .= $page->bodyBuilder->lili("L'avant-projet est payant.");
        $body .= $page->bodyBuilder->lili("Chaque artisan a un contrat.");

        $body .= "</ul>\n";
        $body .= "</div>\n";
    //
        // SUivi du chantier
        $body .= $page->bodyBuilder->titleAnchor("Suivi du chantier", 3);
    ...
    //
        // Reduire les couts
        $body .= $page->bodyBuilder->titleAnchor("R&eacute;duire les co&ucirc;ts", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("si terrasse plus tard, aplanir, tasser, c&acirc;bler");
        $body .= $page->bodyBuilder->lili("pr&eacute;voir la tuyauterie et le lieu pour un aspirateur central avec un une sortie dans chaque pi&egrave;ce et une $aa l'ext&eacute;rieur (par ex. garage)");
        $body .= $page->bodyBuilder->lili("pour une piscine, prendre une pompe $aa chaleur qui peut chauffer l'eau");
        $body .= $page->bodyBuilder->lili("si installation &eacute;lectrique sur la terrasse (store), pr&eacute;voir les tubes pour les c&acirc;bles");
        $body .= "</ul>\n";
        $body .= "</li>\n";
        ...
        $body .= $page->bodyBuilder->lili("jardin/am&eacute;nagements ext&eacute;rieurs simples");
        $body .= $page->bodyBuilder->lili("faire soi-m&ecirc;me certains travaux");
        ...
        $body .= $page->bodyBuilder->lili("Eviter le luxe: sauna/jaccuzzi 10$kchf utilis&eacute; qu'au d&eacute;but; piscine 50$kchf, entretien cher");
        $body .= $page->bodyBuilder->lili("Bourse aux mat&eacute;riaux d'excavations " . getLink("http://vd.ch/boume"));
    ...
    //
        ...
        $body .= $page->bodyBuilder->titleAnchor("De l'avant-projet au permis de construire", 3);
        $body .= "<div>\n";
        $body .= "<ol>\n";
    ...
    //
        // Assurances
        $body .= $page->bodyBuilder->titleAnchor("Assurances", 3, "ConstruireExecution");
        $body .= "<div>\n";
    ...
    //
        ...
        $body .= $page->bodyBuilder->titleAnchor("R&eacute;ception et &eacute;tat des lieux", 3);
    ...
    //
        // Defaut d'ouvrage
        $body .= $page->bodyBuilder->titleAnchor("D&eacute;faut d'ouvrage", 3);
        $body .= "<div>\n";

        $body .= "<p>Cela signifie que l'ouvrage livr&eacute; ne correspond pas au contrat.</p>\n";

        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("<b>D&eacute;faut apparent:</b> imm&eacute;diatement d&eacute;couvert $aa r&eacute;ception, ou aurait d&ucirc; &ecirc;tre d&eacute;couvert imm&eacute;diatement.");
        $body .= $page->bodyBuilder->lili("<b>D&eacute;faut cach&eacute;:</b> ne peut &ecirc;tre d&eacute;couvert que plus tard.");
        $body .= "</ul>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("le prix peut &ecirc;tre diminu&eacute;");
        $body .= $page->bodyBuilder->lili("l'entreprise corrige le d&eacute;faut, mais seulement si c'est possible sans frais exag&eacute;r&eacute;s");
        $body .= "</ul>\n";
    ...
    //
        // Decompte final
        $body .= $page->bodyBuilder->titleAnchor("D&eacute;compte final", 3);
...
//
    ...
    $body .= $page->bodyBuilder->titleAnchor("Contrats et propri&eacute;t&eacute;s");
    $body .= "<div>\n";
    $body .= "<ul>\n";
    ...
    $body .= $page->bodyBuilder->lili("l&eacute;gal (pas de contrat): par ex. descendants, fr&egrave;res/soeurs, paysans, co$proprio");
    ...
    //
        // Notaires
        $body .= $page->bodyBuilder->titleAnchor("Notaires", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("Acte de vente: droits de mutation, inscription au $RF, r&eacute;mun&eacute;ration et frais.");
        $body .= $page->bodyBuilder->lili("Acte hypoth&eacute;quaire: inscription au $RF, r&eacute;mun&eacute;ration.");
        $body .= "</ul>\n";

        $body .= "<p>Pour un bien existant, le contrat sp&eacute;cifie clairement:</p>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("la date de changement de $proprio");
        $body .= $page->bodyBuilder->lili("l'&eacute;tat $aa la remise (propre et vide)");
        ...
        $body .= $page->bodyBuilder->lili("les r&eacute;serves &eacute;ventuelles en cas de d&eacute;chets/pollutions anciens");
        $body .= $page->bodyBuilder->lili("la participation actuelle au fonds de r&eacute;novation de la PPE");
    ...
    //
        // Paiement
        $body .= $page->bodyBuilder->titleAnchor("Paiement", 3);
    ...
    //
        // RF
        $body .= $page->bodyBuilder->titleAnchor("Registre foncier", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("journal: r&eacute;quisitions d'op&eacute;rations inscrites");
        $body .= $page->bodyBuilder->lili("grand livre: ensemble des feuillets");
        $body .= $page->bodyBuilder->lili("plan");
        $body .= $page->bodyBuilder->lili("contrats: achat, servitudes");
        $body .= $page->bodyBuilder->lili("registres accessoires: {$proprio}s, cr&eacute;anciers...");
        $body .= "</ul>\n";

        $body .= "</div>\n";
    //
        // Formes de proriete
        $body .= $page->bodyBuilder->titleAnchor("Formes de propri&eacute;t&eacute;", 3);
        $body .= "<div>\n";
        $body .= "<ul>\n";

        $body .= $page->bodyBuilder->lili("Individuelle: tout seul, mais le conjoint officiel a l&eacute;gallement des droits.");
    ...
    //
        // PPE
        $body .= $page->bodyBuilder->titleAnchor("PPE", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("une quote-part de l'immeuble entier");
        $body .= $page->bodyBuilder->lili("une part priv&eacute;e");
        $body .= "</ul>\n";

        $body .= "<p>L'administrateur (ex&eacute;cutif) g&egrave;re:</p>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("les travaux d'entretien");
        $body .= $page->bodyBuilder->lili("l'ex&eacute;cution des d&eacute;cisions de l'assembl&eacute;e");
        $body .= $page->bodyBuilder->lili("la r&eacute;partition des charges communes, la facturation");
        $body .= $page->bodyBuilder->lili("le budget suivant");
        $body .= $page->bodyBuilder->lili("l'utilisation des moyens financiers");
        $body .= $page->bodyBuilder->lili("les droits sp&eacute;ciaux, l'utilisation des communs");
        $body .= "</ul>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("&eacute;lection/d&eacute;mission et contr&ocirc;le de l'administrateur");
        $body .= $page->bodyBuilder->lili("choix du r&eacute;viseur des comptes");
        $body .= $page->bodyBuilder->lili("approbation des comptes et du budget de r&eacute;partition des charges");
        $body .= $page->bodyBuilder->lili("fixation des montants pour les fonds de r&eacute;novation et la r&eacute;partition des co&ucirc;ts");
        $body .= $page->bodyBuilder->lili("&eacute;laboration/changements de r&egrave;glements");
        $body .= $page->bodyBuilder->lili("d&eacute;cisions pour les travaux der&eacute;novations/transformations");
        $body .= $page->bodyBuilder->lili("d&eacute;cisions administrative hors du cadre de l'administrateur");
        $body .= "</ul>\n";

        $body .= "</div>\n";

            // Fond de renovation
            $body .= $page->bodyBuilder->titleAnchor("Fonds de r&eacute;novation", 4);
        ...
        //
            // Charges
            $body .= $page->bodyBuilder->titleAnchor("Charges", 4);
            $body .= "<div>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("frais d'entretiens/r&eacute;parations");
            $body .= $page->bodyBuilder->lili("concierge");
            $body .= $page->bodyBuilder->lili("eau, &eacute;lectricit&eacute;, chauffage");
            $body .= $page->bodyBuilder->lili("assurances");
            $body .= $page->bodyBuilder->lili("frais g&eacute;n&eacute;raux");
            $body .= $page->bodyBuilder->lili("honoraires de l'administrateur");
            $body .= "</ul>\n";
            $body .= "</div>\n";
        //
            // Reglement
            $body .= $page->bodyBuilder->titleAnchor("R&egrave;glement", 4);
        ...
        //
            ...
            $body .= $page->bodyBuilder->titleAnchor("Contenu du contrat d'achat", 4);
            $body .= "<div>\n";
            $body .= "<ul>\n";
            $body .= $page->bodyBuilder->lili("r&egrave;glement de la communeaut&eacute; de co{$proprio}s");
            $body .= $page->bodyBuilder->lili("r&egrave;glement d'utilisation et d'administration");
            $body .= $page->bodyBuilder->lili("quote-part");
            $body .= "</ul>\n";
            $body .= "</div>\n";
        //
            // Check-list avant achat
            $body .= $page->bodyBuilder->titleAnchor("Check-list avant l'achat", 4);
            $body .= "<div>\n";
            $body .= "<ul>\n";

            $body .= $page->bodyBuilder->lili("Que comprend le lot en vente, quels sont les communs?");
            $body .= $page->bodyBuilder->lili("Conditions pour communs? Utilisation par parents, amis, etc?");
            $body .= $page->bodyBuilder->lili("But? (habitat, bureau, atelier, cabinet...)");
            $body .= $page->bodyBuilder->lili("Possibilit&eacute; de sous-louer?");
            $body .= $page->bodyBuilder->lili("Interdictions des professions immorales?");
            $body .= $page->bodyBuilder->lili("R&egrave;glement pour les loisirs incommodants? (bricolage, musique...)");
            $body .= $page->bodyBuilder->lili("Animaux admis? Place ext&eacute;rieure?");
            $body .= $page->bodyBuilder->lili("R&egrave;glement pour la d&eacute;coration des balcons?");
            $body .= $page->bodyBuilder->lili("Possible de remplacer la moquette par du parquet? (transmission du bruit)");

            $body .= "</ul>\n";
            $body .= "</div>\n";
    //
        // Couple et succession
        $body .= $page->bodyBuilder->titleAnchor("Couple et succession", 3);
...
//
    // Charges et impots
    $body .= $page->bodyBuilder->titleAnchor("Charges et imp&ocirc;ts");

        // Charges en un coup d'oeil
        $body .= $page->bodyBuilder->titleAnchor("Les charges en un coup d'oeil", 3);
        $body .= "<div>\n";

        $body .= "<p>Voici une petite liste de charges qu'un $proprio doit assumer:</p>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("int&eacute;r&ecirc;ts hypot&eacute;caires");
        $body .= $page->bodyBuilder->lili("amortissement de l'$hyp");
        $body .= $page->bodyBuilder->lili("abonnement TV, internet, t&eacute;l&eacute;phone");
        $body .= $page->bodyBuilder->lili("Chauffage et eau chaude");
        $body .= $page->bodyBuilder->lili("Eau");
        $body .= $page->bodyBuilder->lili("Electricit&eacute;");
        $body .= $page->bodyBuilder->lili("Jardin: entretien, tondeuse, semis...");
        $body .= $page->bodyBuilder->lili("Ramoneur");
        $body .= $page->bodyBuilder->lili("Entretien de la chaudi&egrave;re");
        $body .= $page->bodyBuilder->lili("Entretien du br&ucirc;leur et de la citerne");
        $body .= $page->bodyBuilder->lili("Taxe des eaux us&eacute;es");
        $body .= $page->bodyBuilder->lili("Taxe d&eacute;chets");
        $body .= $page->bodyBuilder->lili("Assurances d&eacute;g&acirc;ts d'eau");
        $body .= $page->bodyBuilder->lili("Assurance b&acirc;timent");
        $body .= $page->bodyBuilder->lili("Assurance bris de galce");
        $body .= $page->bodyBuilder->lili("Assurance RC");
        $body .= $page->bodyBuilder->lili("Autres assurances...");
        $body .= $page->bodyBuilder->lili("Imp&ocirc;t foncier");
        $body .= "<li>PPE:\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("Conciergerie");
        $body .= $page->bodyBuilder->lili("Ascenceur");
        $body .= $page->bodyBuilder->lili("Fonds de r&eacute;novation");
        $body .= $page->bodyBuilder->lili("Frais d'administration");
        $body .= "</ul></li>\n";
        $body .= "</ul>\n";
    ...
    //
        // Assurances
        $body .= $page->bodyBuilder->titleAnchor("Assurances", 3, "ChargesEtImpots");
        $body .= "<div>\n";
        $body .= "<ul>\n";

        $body .= "<li><b>Incendie/&eacute;l&eacute;ments naturels:</b>\n";
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("immeuble: assure le b&acirc;timent, obligatoire dans la plupart des cantons");
        $body .= $page->bodyBuilder->lili("loyer/jardin/etc: compl&eacute;mentaires pour d&eacute;g&acirc;ts jardin, ou pour frais de logement provisoire pendant des travaux cons&eacute;cutifs aux d&eacute;g&acirc;ts");
        $body .= "</ul>\n";
        ...
        $body .= $page->bodyBuilder->lili("<b>RC priv&eacute;e:</b> dommages $aa autrui. Pas n&eacute;cessaire si RC b&acirc;timent. {$pasoblvivrec}e.");
        $body .= $page->bodyBuilder->lili("<b>RC b&acirc;timent:</b> pas n&eacute;cessaire si vit dans sa propre maison ou si PPE max 3 logements");
        ...
        $body .= $page->bodyBuilder->lili("<b>Tremblement de terre:</b> les assurances propos&eacute;es en Suisse (2015) ne sont pas terribles, comparer les offres disponibles.");
        $body .= $page->bodyBuilder->lili("<b>APG:</b> faite par l'employeur");
        $body .= $page->bodyBuilder->lili("<b>Assurance vie:</b> pour que les proches touchent un revenu en cas de d&eacute;c&egrave;s");

        $body .= "</ul>\n";
        $body .= "</div>\n";
    //
        // Entretien (frais+provisions)
        $body .= $page->bodyBuilder->titleAnchor("Entretien (frais et provisions)", 3);
        $body .= "<div>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("0.7% pour l'entretien courant");
        $body .= $page->bodyBuilder->lili("0.3% pour les provisions des gros entretiens");
        $body .= "</ul>\n";
        ...
        $body .= "<ul>\n";
        $body .= $page->bodyBuilder->lili("&eacute;pargner 1%");
        $body .= $page->bodyBuilder->lili("&eacute;pargner 0.7% et amortir 0.3%");
        $body .= "</ul>\n";
    ...
    //
        // Impots
        $body .= $page->bodyBuilder->titleAnchor("Imp&ocirc;ts", 3);
    ...
    //
        // Deductions
        $body .= $page->bodyBuilder->titleAnchor("D&eacute;ductions", 3);

            // Interets hypothecaires
            $body .= $page->bodyBuilder->titleAnchor("Int&eacute;r&ecirc;ts hypot&eacute;caires", 4);
        ...
        //
            // Entretien
            $body .= $page->bodyBuilder->titleAnchor("Entretien", 4);
        ...
        //
            // Exploitation
            $body .= $page->bodyBuilder->titleAnchor("Exploitation", 4);
        ...
        //
            // Administration
            $body .= $page->bodyBuilder->titleAnchor("Administration", 4);

            $body .= "<p>Seulement si le bien est lou&eacute;: g&eacute;rance, pub, agence immobili&egrave;re...</p>\n";
        //
            // Planification
            $body .= $page->bodyBuilder->titleAnchor("Planification", 4);
    ...
    //
        ...
        $body .= $page->bodyBuilder->titleAnchor("Imp&ocirc;t sur la fortune immobili&egrave;re", 3);

        $body .= "<p>En tant que $proprio, le bien est compt&eacute; dans la fortune, moins l'$hyp et les int&eacute;r&ecirc;ts.</p>\n";
    //
        // Societe immobiliere
        $body .= $page->bodyBuilder->titleAnchor("Soci&eacute;t&eacute; immobili&egrave;re", 3);
    ...
    //
        // Achat/vente
        $body .= $page->bodyBuilder->titleAnchor("Achat / vente", 3);
...
//
    // Experiences
    $body .= $page->bodyBuilder->titleAnchor("Exp&eacute;riences partag&eacute;es");
    $body .= "<div>\n";
    ...
    $body .= $page->bodyBuilder->lili("SwissLife bon taux, mais on doit conclure une assurance vie chez eux.");
    $body .= $page->bodyBuilder->lili("Pour choisir la banque, regarder aussi o&ugrave; sont les bancomat et si Twint est disponible.");
    $body .= $page->bodyBuilder->lili("Attention aux limites de retrait au moment d'amener les fonds propres!");
    $body .= $page->bodyBuilder->lili("Comparer les notaires avec les amis.");

    $body .= $page->bodyBuilder->lili("Faire un plan $aa l'&eacute;chelle avec toutes les pi&egrave;ces y compris les ext&eacute;rieurs");
    $body .= $page->bodyBuilder->lili("Calculer le co&ucirc;t d'une salle de bain, d'une cuisine: frigo 2$kchf, four 2$kchf, plaques 4$kchf, lave-vaisselle 1$kchf, hotte 1$kchf - total 20$kchf");
    $body .= $page->bodyBuilder->lili("construire est cher, meilleur march&eacute; de r&eacute;nover");
    $body .= $page->bodyBuilder->lili("5% cash en plus pour frais. Si pas construit, taxes seulement sur prix du terrain, mais rajouter taxes raccordement...");
    $body .= $page->bodyBuilder->lili("taux fixe 5ans max? Faire estimation avec limite et marge");
    $body .= $page->bodyBuilder->lili("Surface de fen&ecirc;tres: 1/8 $aa 1/6");
    $body .= $page->bodyBuilder->lili("PPE: frais en plus 500-1$kchf");
    $body .= $page->bodyBuilder->lili("PPE plus: r&eacute;partitions des frais communs (par ex. toit); moins: plusieurs $aa d&eacute;cider, on choisit pas les autres");
    $body .= $page->bodyBuilder->lili("Penser $aa faire une assurance vie pour que le survivant puisse encore payer le bien");
    $body .= "</ul>\n";

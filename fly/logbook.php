...
if($userIsAdmin) {
    if(isset($_POST["add"])) {
        ...
        $formTimeStart = $_POST["start_time"];
        $formTimeStop = $_POST["stop_time"];

        // Get rid of seconds (if any)
        if(strlen($formTimeStart) == 8) {
            $formTimeStart = substr($formTimeStart, 0, 5);
        }
        if(strlen($formTimeStop) == 8) {
...
$attrSize->size = $formsize;
    ...
    //
        ...
        if($userIsAdmin) {
            ...
            //
                // notes
                $noteAttr = new FieldAttributes();
                $noteAttr->size = 50;
                $tinsert .= $page->butler->cell($theTextInput->get("notes", $formNotes, "", NULL, $noteAttr), array("class" => "notes"));

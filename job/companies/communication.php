...
if(isset($_POST["erase"])) {
	...
	$query = "DELETE FROM `{$page->dbHelper->dbName}`.`comco` WHERE `comco`.`id` = ? LIMIT 1;";
	...
	if(isset($_POST["id"])) {
		// update
		$id = $_POST["id"];
		$query = "UPDATE `{$page->dbHelper->dbName}`.`comco` SET `timestamp` = ?, `who` = ?, `media` = ?, `way` = ?, `kind` = ?, `content` = ? WHERE `comco`.`id` = ? LIMIT 1;";
		$sql = $page->dbHelper->queryPrepare($query);
		...
		// insert
		$query = "INSERT INTO `{$page->dbHelper->dbName}`.`comco` (`company`, `timestamp`, `who`, `media`, `way`, `kind`, `content`) VALUES(?, ?, ?, ?, ?, ?, ?);";
		$sql = $page->dbHelper->queryPrepare($query);
	...
	$page->htmlHelper->headerLocation("display.php?id=$company");
	$sql = $page->dbHelper->idManage("SELECT * FROM `companies` WHERE `id` = ?", $company);
	...
	$id = $_GET["id"];
	$SQL = $page->dbHelper->idManage("SELECT * FROM `comco` WHERE `id` = ?", $id);
	$SQL->bind_result($id, $company, $timestamp, $who, $media, $way, $kind, $content);
	$SQL->fetch();
	$SQL->close();
	$who = $page->dbText->sql2field($who);
...
$body .= $page->formHelper->subButt($id > 0, "la communication #$id avec $company");
